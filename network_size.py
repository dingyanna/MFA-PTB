...
def network_size(args):
    '''  
    RQ: How does the error of step 1(mfa) change w.r.t network sizes? 
    '''
    # setup logger 
    ...
    log_path = f'{args.save_dir}/result_{args.avg_operator}_{args.dynamics}.log'
    ...
    N_lis = [100, 200, 400, 600, 800, 1000, 2000, 4000, 6000, 8000 ]
    ...
    for k in range(len(N_lis)):
        ...
        for r in range(5): 
            ...
            # True Steady State
            true_steady_state_runtime, residual, true, full_deriv = full(net, args, logger,SOLVER=SOLVER) 
    ...
    df = pd.DataFrame(dic)
    df.to_csv(f'{args.save_dir}/result_{args.avg_operator}_{args.dynamics}.csv', index=False)
